...
function fireConfetti() {
  const colors = ["#ffb45e", "#f59a23", "#3fd6c0", "#8fd9e8", "#dbe4ee"];
...
/** Таймер занимает максимум экрана; размер зависит от числа видимых блоков. */
function sizeFor(unitCount: number): string {
  if (unitCount === 4) return "text-[min(11.5vw,16.5vh,11rem)]";
  if (unitCount === 3) return "text-[min(15vw,17vh,12rem)]";
  return "text-[min(23vw,18vh,13rem)]";
}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const schedule = () => {
      glitchTimer.current = window.setTimeout(() => {
        setGlitchPulse((v) => v + 1);
        const burst: GlitchBar[] = Array.from(
        ...
        setBars(burst);
        window.setTimeout(() => setBars([]), 420);
        schedule();
      }, 3600 + Math.random() * 5600);
    };
    schedule();
    return () => window.clearTimeout(glitchTimer.current);
  }, []);
  ...
  const glitching = glitchPulse > 0;
  const glitchKey = `${glitchPulse}`;
      ...
      >
        <div className="scene-bg noise scanlines relative flex h-full flex-col overflow-hidden">
          ...
          {[
            ...
            <span
              key={pos}
              className={`pointer-events-none absolute z-10 ${pos} font-mono text-sm ${
                i % 2 ? "text-ember/40" : "text-dim/60"
              }`}
              aria-hidden="true"
            >
              +
          ...
          <header className="fade-up relative z-10 flex items-center justify-between gap-4 px-6 py-5 sm:px-10">
            <div className="flex items-center gap-3">
              <span className="relative flex h-9 w-9 items-center justify-center rounded-md border border-line bg-panel text-ember">
                ...
                <span className="absolute -right-1 -top-1 h-2 w-2 rounded-full bg-lagoon" />
              </span>
          ...
          <main className="relative z-10 flex min-h-0 flex-1 flex-col items-center justify-center px-4">
            ...
            >
              ...
              >
                ...
                />
                <path d="M100 1v8M100 191v8M1 100h8M191 100h8" stroke="#39434e" strokeWidth="1" />
              </svg>
            </div>
                  ...
                  {phase === "finished" ? (
                    <div className="flex flex-col items-center gap-4">
                      ...
                      <TimerDigits
                        ...
                        alarm
                        glitching={glitching}
                        sizeClass="text-[clamp(1.6rem,6vw,3.4rem)] opacity-60"
                      />
                    </div>
                  ) : (
                    <TimerDigits
