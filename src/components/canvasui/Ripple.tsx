/**
 * Ripple — Canvas UI (https://canvasui.dev)
 * Water ripples that spread from every click and refract the live page
 * like a pond surface, with dispersion and light glints on the crests.
 * In browsers without html-in-canvas support it degrades to a pure
 * WebGL glint overlay — content stays regular, interactive HTML.
 */
import {
  useEffect,
  useRef,
...
export function Ripple({ children, className, style, ...options }: RippleProps) {
  ...
  const [initialOptions] = useState<RippleOptions>(options);
